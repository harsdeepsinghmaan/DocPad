...
- (id) initWithDelegate:(id<UHNBGMControllerDelegate>) delegate;
{
    ...
    NSArray *requiredServices = @[kGlucoseServiceUUID, kDEVICE_INFO_SERVICE_UUID];
...
- (void) getNumberOfRecordsGreaterThan:(int)number {
    NSNumber *num = [NSNumber numberWithInt:number];
    self.numberOfRecordsReceived = 0;
    NSData *command = [NSData reportStoredRecordsGreaterThanOrEqualToTimeOffset:(UInt16)[num unsignedCharValue]];
    [self sendRACPCommand:command];
}
...
- (void) bleController:(UHNBLEController *) controller didDiscoverCharacteristics:(NSArray *) characteristicUUIDs forService:(NSString *) serviceUUID;
{
    ...
    if ([serviceUUID isEqualToString:kGlucoseServiceUUID])
    {
        ...
        if ([self.delegate respondsToSelector:@selector(bgmController:didConnectToGlucoseMeterWithName:withIdentifier:)])
        {
            [self.delegate bgmController:self didConnectToGlucoseMeterWithName:self.bgmDeviceName withIdentifier:self.deviceIdentifier.UUIDString];
        }
                
    }
    else if([serviceUUID isEqualToString:kDEVICE_INFO_SERVICE_UUID]) {
        if ([characteristicUUIDs containsObject:@"2A25"]) {
                [self.bleController readValueFromCharacteristicUUID:kGlucoseServiceCharacteristicSerialNo withServiceUUID:kDEVICE_INFO_SERVICE_UUID];
...
- (void) bleController:(UHNBLEController *) controller didUpdateValue:(NSData *) value forCharacteristic:(NSString *) charUUID;
{
    ...
    } else if ([charUUID isEqualToString:kGlucoseServiceCharacteristicSerialNo]) // 0x2A25

     {

      NSString * serialNumber = [[NSString alloc] initWithData:value encoding:NSUTF8StringEncoding];

      NSLog(@"   Serial Number = %@", serialNumber, nil);

         [self handleNotificationStateUpdateToSerialNumber:serialNumber];
...
- (void) handleNotificationStateUpdateToSerialNumber:(NSString*) SerialNumber;
{
    // if we need to enable all notifications, then enable them all
    if ([self.delegate respondsToSelector:@selector(bgmController:didGetSerialNumber:)]) {
        [self.delegate bgmController:self didGetSerialNumber:SerialNumber];
    }
}
