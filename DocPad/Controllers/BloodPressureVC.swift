...
class BloodPressureVC: UIViewController ,UITableViewDelegate,UITableViewDataSource {
    ...
    func getBloodpressureApi(getData: Int){
        hud.show(animated: true)
        let headers: [String:Any] = ["X-Auth-Token": self.authToken]
        personID = UserDefaults.standard.value(forKey: "PersonID") as! Int
        let request = NSMutableURLRequest(url: NSURL(string: "\(url)/bloodpressure/pagination/\(personID)?currentPage=\(getData)&pageSize=30&sortFields=datenTime&sortDirections=desc")! as URL,
                                ...
                                if dict != nil {
                                    ...
                                    if self.newDataArray.count != 0{
                                        do{
                                            ...
                                            UserDefaults.standard.set(jsonData, forKey: "BpDict")
...
//                                            var jsonData = Data()
//                                            if #available(iOS 11.0, *) {
//                                                jsonData = try NSKeyedArchiver.archivedData(withRootObject: self.arrayLevel_All_Data, requiringSecureCoding: true)
//                                            } else {
//                                                // Fallback on earlier versions
//                                                jsonData =  NSKeyedArchiver.archivedData(withRootObject: self.arrayLevel_All_Data)
//                                            }
//                                            UserDefaults.standard.set(jsonData, forKey: "BpDict")
//                                            UserDefaults.standard.synchronize()
//
...
//                                    }
            ...
            for section in sections {
                ...
                if section == arrayLevel_All_Data.count - 1{
                    if(!isPageRefresing){ // no need to worry about threads because this is always on main thread.
                        ...
                        getBloodpressureApi(getData:fromIndex)
                    }
                }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "BloodPressureCell") as! BloodPressureCell
        ...
        if var note = ((search_parent_Array[indexPath.row] as AnyObject).value(forKey: "note") as? String){
            cell.txtVW.text = note
        ...
        systolicIntVal = ((search_parent_Array[indexPath.row] as AnyObject).value(forKey: "systolic") as? Int ?? 0)
        diastolicIntVal = ((search_parent_Array[indexPath.row] as AnyObject).value(forKey: "diastolic") as? Int ?? 0)
        pulseIntVal = ((search_parent_Array[indexPath.row] as AnyObject).value(forKey: "pulse") as? Int ?? 0)
        var systolic = String(systolicIntVal)
        var diastolic = String(diastolicIntVal)
        var pulse = String(pulseIntVal)
        cell.lblSystolic.text = systolic
       cell.lblDiastolic.text = diastolic
        cell.lblPulse.text = pulse
...
//        let timeStamp = (search_parent_Array[indexPath.row] as AnyObject).value(forKey: "datenTime") as! Int
        let timeDouble = Double(timeStamp)
...
//        if var note = ((self.dataArray[indexPath.row] as AnyObject).value(forKey: "note") as? String){
    ...
    func tableView(_ tableView: UITableView,
        ...
        if let deviceID = (array3[indexPath.row] as AnyObject).value(forKey: "deviceId") as? String {
            if deviceID != "" {
                ...
                let controller = storyboard?.instantiateViewController(withIdentifier: "AdditionalBloodPressureVc") as! AdditionalBloodPressureVc
...
//        }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            bpId = ((array3[indexPath.row] as AnyObject).value(forKey: "bpId") as? Int)!
            systolic = ((array3[indexPath.row] as AnyObject).value(forKey: "systolic") as? Int)!
            diastolic = ((array3[indexPath.row] as AnyObject).value(forKey: "diastolic") as? Int)!
            pulse = ((array3[indexPath.row] as AnyObject).value(forKey: "pulse") as? Int)!
            ...
            callDeleteApi(_bpId: bpId, _systolic: systolic, _datenTime: datenTime, _diastolic: diastolic, _note: note, _personID: personID, _enteredBy:enteredBy, _source:source, _pulse: pulse )
    ...
    func callDeleteApi(_bpId: Int,_systolic: Int,_datenTime: NSNumber,_diastolic: Int,_note: String,_personID: Int,_enteredBy: Int,_source: String,_pulse: Int ){
        hud.show(animated: true)
        ...
        let request = NSMutableURLRequest(url: NSURL(string: "\(url)/bloodpressure/\(_bpId)")! as URL,
                                          cachePolicy: .useProtocolCachePolicy,
                                          timeoutInterval: 100.0)
                            ...
                            DispatchQueue.main.async {
                                ...
                                if json == "true" {
                                    ...
                                    let alert = UIAlertController(title: "Successs", message:
                                        "Blood Pressure object has been deleted successfully", preferredStyle: UIAlertController.Style.alert)
    ...
    @IBAction func btnAdd(_ sender: Any) {
        let vc = storyboard?.instantiateViewController(withIdentifier: "AdditionalBloodPressureVc") as! AdditionalBloodPressureVc
