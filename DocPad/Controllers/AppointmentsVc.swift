...
class AppointmentsVc: UIViewController ,UICollectionViewDelegateFlowLayout, UICollectionViewDelegate,UICollectionViewDataSource{
    ...
    var itemNameArray = ["Request Appointment","View Appointments"]
    var itemImageArray = [#imageLiteral(resourceName: "requestAppointment"),#imageLiteral(resourceName: "viewAppointment")]
    ...
    {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "appointmentCell", for: indexPath) as! AppointmentsCvc
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if ((indexPath.row == 0) && (indexPath.section == 0)){
            let controller = storyboard?.instantiateViewController(withIdentifier: "RequestAppoinmentVc") as! RequestAppoinmentVc
            self.navigationController?.pushViewController(controller, animated: true)
        }
        else  if ((indexPath.row == 1) && (indexPath.section == 0)){
            let controller = storyboard?.instantiateViewController(withIdentifier: "ViewAppointmentsVc") as! ViewAppointmentsVc
            self.navigationController?.pushViewController(controller, animated: true)
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if (screenWidth == 320){
            let cellSize = CGSize(width: ((self.view.frame.size.width/3) - 20), height: 150)
            return cellSize
        }
        else{
