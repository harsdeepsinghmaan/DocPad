...
characteristicUUID:[CBUUID UUIDWithString:SystemID_Char]
                p:self.activePeripheral];
    [self readValue:[CBUUID UUIDWithString:DeviceInformation_Service]
    characteristicUUID:[CBUUID UUIDWithString:SerialNumber_Char]
                    p:self.activePeripheral];
}
...
- (void)readMeasurementForSetupBp
{
    ...
    characteristicUUID:[CBUUID UUIDWithString:BloodPressureMeasurement_Char]
                     p:self.activePeripheral
                    on:YES];
    [self notification:[CBUUID UUIDWithString:DeviceInformation_Service]
    characteristicUUID:[CBUUID UUIDWithString:SerialNumber_Char]
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
  if (!error) {
    ...
    for(int i=0; i < service.characteristics.count; i++) {
      CBCharacteristic *c = [service.characteristics objectAtIndex:i];
        if ((c.properties&CBCharacteristicPropertyRead) && [c.UUID isEqual:[CBUUID UUIDWithString:SerialNumber_Char]])
         {
              [peripheral readValueForCharacteristic:c];
         }
        if ((c.properties&CBCharacteristicPropertyNotify) && [c.UUID isEqual:[CBUUID UUIDWithString:SerialNumber_Char]]) {
            [peripheral setNotifyValue:true forCharacteristic:c];
        }
        NSLog(@"Found characteristic %s %@\r\n",[ self CBUUIDToString:c.UUID], service.UUID);
...
- (int) findBLEPeripherals {
  ...
  _shouldScan = YES;
    if (self.CM.state  == CBCentralManagerStatePoweredOn) {
       // if(![[NSUserDefaults standardUserDefaults] boolForKey:@"BloodPressure"]) {
            
        [self.CM scanForPeripheralsWithServices:[NSArray arrayWithObjects:[CBUUID UUIDWithString:WeightScale_Service],[CBUUID UUIDWithString:BloodPressure_Service], nil] options:nil];
        
    //[self.CM scanForPeripheralsWithServices:[NSArray arrayWithObject:[CBUUID UUIDWithString:WeightScale_Service]] options:nil];
...
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Device info"
//                                                        message:[NSString stringWithFormat:@"charUUID is %@", charUUID]
//                                                       delegate:nil
...
//        [alert show];
  ...
  if (!error) {
    if ([charUUID isEqual:[CBUUID UUIDWithString:ManufacturerNameString_Char]]) {
    ...
    } else if ([charUUID isEqual:[CBUUID UUIDWithString:SerialNumber_Char]]) {
        self.serialNumber = [[NSString alloc] initWithData:characteristic.value encoding:NSUTF8StringEncoding];
        [self.delegate gotSerialNumberWeight:self.serialNumber];
      ...
      self.data = [[NSData alloc] initWithData:characteristic.value];
        ...
        if (bpmData.length >= 12) { //If time has been set during pairing
            ...
            [data setValue:self.time forKey:@"time"];
            [data setValue:self.serialNumber forKey:@"serialNumber"];
      ...
      NSData *weightData = [[NSData alloc] initWithData:characteristic.value];
        if (weightData.length > 3) {
            ...
            [data setValue:self.time forKey:@"time"];
            if (self.serialNumber == nil || self.serialNumber == [NSNull null]) {
                self.serialNumber = @"";
            }
            [data setValue:self.serialNumber forKey:@"serialNumber"];
